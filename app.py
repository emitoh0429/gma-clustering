...
def optimize():
    try:
        ...
        # Small tie-breaker penalties
        # These are not real costs; they just encourage cleaner schedules.
        ...
        # Constraint 3:
        # Actor linking.
        # If actor appears in any scene on a day, pay actor once that day.
        for actor in all_actors:
            related_scenes = [
        ...
        # If several detailed locations share one broad Location,
        # charge that broad Location only once per day.
        for loc in all_locations:
            related_scenes = [
                i for i in scene_indices
        ...
        for d in days:
            if pulp.value(y[d]) is None or pulp.value(y[d]) < 0.5:
                continue
            ...
            # Detailed locations used for readable one-liner output
            ...
            schedule.append({
                ...
                # One-liner readable field
                "oneliner": (
                    f"Day {len(schedule) + 1} | "
                ...
                # Scene-level detail for production-style output
                "scene_details": [
                    {
